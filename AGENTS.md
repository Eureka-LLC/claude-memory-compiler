...
| `layers` | число | Сколько append-слоёв «## Обновление ДД» накоплено в статье. **Проставляется автоматически** (`compile`/`consolidate`) — заполнять не нужно. При `layers ≥ 4` lint предлагает консолидацию. |
...
#### Обновление статей: append-слои, не перезапись

Когда новый день дополняет существующую статью, compile её **НЕ перезаписывает**, а дописывает
датированный слой `## Обновление ГГГГ-ММ-ДД` в конец (через `<<<APPEND>>>`). Так старое тело никогда
не теряется — перезапись через LLM систематически роняла конкретику (номера, код, ссылки). Поле `layers`
считает слои; при `layers ≥ 4` `consolidate.ps1` схлопывает их в единую статью с детерминированной
проверкой покрытия (ни один факт-токен не должен пропасть). `<<<WRITE>>>` остаётся только для НОВЫХ статей.
