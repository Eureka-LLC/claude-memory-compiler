...
function Get-ProjectLabel([string]$FolderName) {
    ...
    # Strategy:
    #   1. Strip drive prefix (X--)
    #   2. Split on 2+ dashes — these encode real path separators (deep paths)
    #   3. If multiple segments: last one is the project name
    #   4. If single segment (shallow path "Username-project"): strip first word
